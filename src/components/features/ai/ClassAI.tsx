...
import { Sparkles, Bot, ArrowRight, ChevronDown } from "lucide-react";
...
export function ClassAI({ onClose, user, classes }: ClassAIProps) {
    ...
    const { messages, input, setInput, loading, sendMessage, setMessages } = useAIChat({
    ...
    const handleSend = async () => {
        const context = `
            ...
            - Danh sách lớp hiện tại: ${classes?.map((c: any) => `${c.name} (Môn: ${c.subject}, Mã: ${c.code})`).join(', ')}
        `;
    ...
    return (
        <AIPanelShell onClose={handleClose} title="Trợ lý Lớp học" loading={loading} sourceContext="class">
            ...
            <div
                ...
                className={`flex-1 flex flex-col px-6 py-4 space-y-6 bg-white scrollbar-thin scrollbar-thumb-gray-100 ${messages.length > 0 ? 'overflow-y-auto' : 'overflow-hidden'}`}
            >
                {messages.length === 0 && (
                    <div className="flex flex-col items-center justify-center h-full text-center space-y-8 opacity-90 pb-20">
            ...
            <div className="px-6 pt-4 bg-white/50 backdrop-blur-sm pb-10">
