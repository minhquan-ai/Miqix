...
export function ScheduleAI({ onClose, weekStartStr, onEventAdded }: ScheduleAIProps) {
    ...
    const { messages, input, setInput, loading, setLoading, sendMessage, setMessages } = useAIChat({
        fetcher: async (userMsg, _, history) => {
            ...
            // Call server action
            const result = await analyzeScheduleAIAction(weekStartStr, userMsg, history, mode);

            // Side effects for options/actions
            ...
            return result; // Returns { message, options, actions }
        },
