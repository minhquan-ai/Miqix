...
4. KIỂM TRA DỮ LIỆU: Chỉ dựa vào dữ liệu được cung cấp.
5. DUY TRÌ NGỮ CẢNH: Đọc kỹ "LỊCH SỬ TRÒ CHUYỆN".`;
...
`;
    ...
    try {
        // Use Unified AICore with JSON Mode
        const parsed = await AICore.generateJSON<any>(fullPrompt, systemPrompt);

        if (!parsed) {
            // Fallback if structured JSON failed but maybe text generation works? 
            // Actually generateJSON returns null on failure so we handle it below.
            throw new Error("Empty response from AI");
        }

        return {
            message: parsed.message || "Mình đã xử lý xong yêu cầu của bạn.",
